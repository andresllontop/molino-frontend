...
function changeLayout($layout, $init) {
    ...
    $body.removeClass('dt-layout--boxed dt-layout--full-framed').addClass('dt-layout--full-width')
    localStorage.setItem('dt-layout', "full-width");
    localStorage.setItem('dt-layout', $layout);
    $(document).trigger('layout-changed');

    if (!$init)
        notifyUser('El tema se cambió exitosamente!.');
}
...
function changeTheme($theme, $style, $themeStylesheet, $init) {
    ...
    if ($theme === 'light') {
        $logo.attr('src', getHostFrontEnd() + 'assets/images/logo/13.png');
        $body.removeClass('theme-dark').removeClass('theme-semidark');
        $themeStylesheet.href = getHostFrontEnd() + 'assets/css/' + $theme + '-' + 'style-6.min.css';
    } else if ($theme === 'dark') {
        $logo.attr('src', getHostFrontEnd() + 'assets/images/logo/14.png');
        $body.removeClass('theme-semidark').addClass('theme-dark');
        $themeStylesheet.href = getHostFrontEnd() + 'assets/css/' + $theme + '-style-1.min.css';

        $('#theme-style-chooser').hide();
    } else if ($theme === 'semidark') {
        $logo.attr('src', getHostFrontEnd() + 'assets/images/logo/14.png');
        $body.find('.dt-login__content-inner .dt-brand__logo-img').attr('src', getHostFrontEnd() + 'assets/images/logo/12.png');
        $body.removeClass('theme-dark').addClass('theme-semidark');
        $themeStylesheet.href = getHostFrontEnd() + 'assets/css/' + $theme + '-' + $style + '.min.css';
    }
