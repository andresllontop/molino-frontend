...
var processAjaxEntrada = () => {
    ...
    xhr.onload = () => {
        ...
        if (beanCrudResponse.list !== undefined) {
            beanPaginationEntrada = beanCrudResponse;
...
function toListEntrada(beanPagination) {
    ...
    beanPagination.list.forEach((detalle) => {
        ...
        addClass($divItem, "dt-contact px-2 click-selection-presentacion");
        ...
        $spanSubItem.textContent = "Total Ingresada: " + detalle.entrada.total + " " + detalle.presentacion.ubicacion_producto.producto.unidad_ingreso.abreviatura;
        $span2SubItem.textContent = "& " + detalle.entrada.total_otro + " " + detalle.presentacion.ubicacion_producto.producto.unidad_salida.abreviatura;
...
function findByPresentacionC(idpresentacion) {
    let presentacion_;
    beanPaginationEntrada.list.forEach((detalle) => {
        if (
            parseInt(idpresentacion) == parseInt(detalle.presentacion.idpresentacion) &&
            parseInt(idpresentacion) > 0
        ) {
            detalle.presentacion.existencia = detalle.entrada.total;
            detalle.presentacion.existencia_otro = detalle.entrada.total_otro;
