...
document.addEventListener('DOMContentLoaded', function () {
    ...
    beanRequestSalida.operation = 'ventas/paginate';
    ...
    document.querySelector('#btnAbrirNewSalida').onclick = function () {
        ...
        removeClass(document.querySelector('#FrmPresentacionC'), "d-none");
        removeClass(document.querySelector('#FrmPresentacionC').nextElementSibling, "d-none");
        removeClass(document.querySelector('#FrmPresentacionC').previousElementSibling, "d-none");
        addSalida();
        listDetalleSalida = [];
    ...
    document.querySelector("#btnCambiarPagoSalida").onclick = (e) => {
...
var processAjaxSalida = () => {
    ...
    if (
        ...
        listPagoVenta.push(new PagoVenta(
            ...
            (document.querySelector("#btnCambiarPagoSalida").textContent.toUpperCase().charAt(0) == 'P') ? 1 : 2,
            null
        ));
        json = {
            salida: new Salida(
                ...
                parseInt(1),
                parseFloat(limpiar_campo(document.querySelector("#txtMontoPagado").value))
            ),
...
var newSelected = () => {
    ...
    if (presentacionSelected.existencia <= 0) {
        return showAlertTopEnd('info', 'El producto no cuenta con Stock');
    }
    ...
    presentacionSelected.existencia = (presentacionSelected.existencia == 0) ? 0 : (presentacionSelected.existencia - 1);
    /* OPERACION AGREGAR*/
    listDetalleSalida.push(
        new Detalle_Salida(
            ...
            (presentacionSelected.existencia == 0) ? 0 : 1,
            (presentacionSelected.existencia == 0) ? 1 : 0,
...
var cantidadDetalleSalida = (eleme, btn) => {
    ...
    if (salidaSelected.presentacion.existencia_inicial < parseInt(btn.value)) {
...
var cantidadOtroDetalleSalida = (eleme, btn) => {
    ...
    if (salidaSelected == undefined) return false;
    if (salidaSelected.presentacion.existencia_inicial_otro < parseInt(btn.value)) {
        showAlertTopEnd('info', 'Cantidad excedida, no hay los suficientes Productos en el almacén.');
        btn.value = 1;
        return false;
    }
        ...
        <div class="input-group">
          ...
          <div class="input-group">
            ...
            <span class="dt-widget__title ml-2 align-self-center" >x ${detalle_salida.presentacion.ubicacion_producto.producto.unidad_ingreso.abreviatura}</span>
				...
				<!-- /widget info -->
				<!-- Widget Info -->
				<div class="dt-widget__info  text-right pl-2 pr-2" style=" max-width: 180px;min-width: 135px;">
            ...
            <span class="dt-widget__title ml-1 align-self-center" >${detalle_salida.presentacion.ubicacion_producto.producto.unidad_ingreso.abreviatura}</span>
				...
				<!-- /widget info -->
				<!-- Widget Info -->
				<div class="dt-widget__info  text-right pl-2 pr-2 d-none" style=" max-width: 180px;min-width: 135px;">
					<div class="input-group search-box right-side-icon">
						<div class="input-group-prepend" style="height: 27px;">
...
function totalMontoRellenar() {
    document.getElementById('txtMontoPagado').value = totalPrecioByDetalleSalida().toFixed(2);
    document.getElementById('txtTotalPagado').innerText = numeroConComas(totalPrecioByDetalleSalida().toFixed(2));
}
